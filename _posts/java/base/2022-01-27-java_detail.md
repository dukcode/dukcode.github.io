...
### Wrapper 클래스
* `String`을 해당 타입으로 변환시키기 위해 과거엔 `parseInt()`와 같은 parse 시리즈의 메서드를 많이 사용했지만, `String`클래스의 `valueOf()`메서드와 이름을 통일하기 위해 같은 기능을 하는 `valueOf()`를 추가하였다.

* `parseInt()`나 `valueOf()`를 사용하기전 `trim()`메서드를 사용하는것을 습관화하자.

```java
Integer.parseInt(" 123 ".trim());
Integer.valueOf(" 123 ".trim());
```
