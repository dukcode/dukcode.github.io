...
```
upstream api {
    server 172.17.0.1:$ENV{"EXTERNAL_PORT"};
}
...
2번째 줄의 `EXTERNAL_PORT`의 값이 환경변수로 부터 `deploy.sh`에서 담길 예정이다. 80포트를 어느 포트에 연결해 줄것인지 결정한다. blue가 켜질 경우 blue에 맞는 외부포트와 연결되고, green일 경우 green에 맞는 외부포트와 연결되고 reload를 통해 새로운 컨테이너를 nginx가 바라보게 한다.
...
이제 `nginx.conf` volume으로 복사하고 Nginx를 `reload`한다. `reloadㅓ` 후에는 기존에 동작하던 컨테이너를 내리고 배포를 종료한다.
