...
다음의 코드는 다음의 역할을 한다. 초대장이 있는 관객에게는 초대장을 티켓으로 교환해주고, 초대장이 없는 관객에게는 돈을 받고 티켓을 판매한다.  **재대로 동작하기는 하지만 변경에 용이하지 않고, 이해하기 쉽지 않다.** 그 이유를 알아보고 코드를 개선해 보자.
